...
#--------------------------------------------------#
from Part_3_utils import clean_text, clean_text_preproc, clean_text_LM, beautiful_print
...
'''
BERT-based model trained on Dataset #0 (from Part 1) and tested on Dataset #1 and Dataset #2 (from Part 2).
Implement this model in Part 3 as a baseline to VADER-Sentiment, DistillBERT-Sentiment, and Plutchik-Emotions Identifier Models.
...
"""
This section defines several functions that can be useful in training the BERT-based Model.
- Function `process_review` cleans the text data in the comments.
- Class `LoadDataset` would load dataset into dataload for training.
- Function `create_data_loader` creates dataload based on dataset.
- Function train_epoch 
- Function eval_model  

"""

# Function to preprocess every tweet
...
def create_data_loader(df, tokenizer, max_len, batch_size):
    ...
    # Returns dataloaders with what ever batch size we want
...
# Write a function to train our model on one epoch
def train_epoch(model, data_loader, optimizer, device, scheduler, n_examples):

    model = model.train()    # tells your model that it is being trained.
    losses = []
    ...
    for idx, d in enumerate(data_loader):
        ...
        optimizer.step()                                            # After gradients are computed by loss.backward() this makes the optimizer 
                                                                    #    iterate over all parameters it is supposed to update and use internally 
                                                                    #    stored grad to update their values.
        scheduler.step()                                            # Make sure learning rate changes. 
                                                                    #    If we dont provide this learning rate stays at initial value.
...
# write a function to validate our model on one epoch
def eval_model(model, data_loader, device, n_examples):

    model = model.eval()   # tells model it shall be in validation mode
    losses = []
    correct_predictions = 0
...
if __name__ == "__main__":
    ...
    # load pre-trained Distill BertTokenizer
    tokenizer = DistilBertTokenizer.from_pretrained(PRE_TRAINED_MODEL_NAME)

    # use below text to understand tokenization process
    # First I am processing our review using above defined function
    sample_text = process_review(df['review_processed'][0])

    # apply our BertTokenizer on sample text
    ...
    encoding = tokenizer.encode_plus(sample_text                             ,
                                     max_length             =  256           ,   # Here for experiment I gave 32 as max_length
    ...
    # Build classifier for our reviews now. 
    # Below n_classes would be 2 in our case since 
    # Classiify review as either positive or negative.
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_acc:
            ...
            best_acc = val_acc
        # Store state of best model indicated by highest validation accuracy


    #====================================================================================================#
    # Load trained model and Test. (Sample code only, test result is meaningless.)
