...
def Load_dataset_3():
    ...
    '''
    # comments_list     = df_raw_3['comments'    ].values.tolist()
    # anger_list        = df_raw_3["anger"       ].values.tolist()
    # anticipation_list = df_raw_3["anticipation"].values.tolist()
    # disgust_list      = df_raw_3["disgust"     ].values.tolist()
    # fear_list         = df_raw_3["fear"        ].values.tolist()
    # joy_list          = df_raw_3["joy"         ].values.tolist()
    # love_list         = df_raw_3["love"        ].values.tolist()
    # optimism_list     = df_raw_3["optimism"    ].values.tolist()
    # pessimism_list    = df_raw_3["pessimism"   ].values.tolist()
    # sadness_list      = df_raw_3["sadness"     ].values.tolist()
    # surprise_list     = df_raw_3["surprise"    ].values.tolist()
    # trust_list        = df_raw_3["trust"       ].values.tolist()

    # all_sentiment_list = ["anger_list"        ,
    #                       "anticipation_list" ,
    #                       "disgust_list"      ,
    #                       "fear_list"         ,
    #                       "joy_list"          ,
    ...
    #     for jdx, one_list_name in enumerate(all_sentiment_list):
    #         if locals()[one_list_name][idx] == 1:
    #             new_comments_list.append(comments)
    #             sentiment_list.append(jdx)
    #             locals()["new_" + one_list_name].append(1)
    #             for one_list_name_x in all_sentiment_list:
    #                 if one_list_name_x != one_list_name:
    #                     locals()["new_" + one_list_name_x].append(0)

    # df_raw_3_adj_dict = {
    #                      'comments'     : new_comments_list     , 
    #                      "anger"        : new_anger_list        , 
    #                      "anticipation" : new_anticipation_list , 
    #                      "disgust"      : new_disgust_list      , 
    #                      "fear"         : new_fear_list         , 
    #                      "joy"          : new_joy_list          , 
    #                      "love"         : new_love_list         , 
    #                      "optimism"     : new_optimism_list     , 
    #                      "pessimism"    : new_pessimism_list    , 
    #                      "sadness"      : new_sadness_list      , 
    #                      "surprise"     : new_surprise_list     , 
    #                      "trust"        : new_trust_list        , 
    #                      "sentiment"    : sentiment_list
    #                      }
    
    # df_raw_3_adj = pd.DataFrame(df_raw_3_adj_dict)
...
if __name__ == "__main__":
    ...
    for one_sentiment in all_sentiment:

        df = df_raw_3.rename(columns={"comments": "review", one_sentiment : "sentiment"})
        ...
        encoding = tokenizer.encode_plus(sample_text                            ,
                                        max_length             =  128           ,   # Here for experiment I gave 32 as max_length
                                        ...
                                        return_token_type_ids  =  False         ,   # since we are dealing with only one sentence 
                                                                                        # as opposed to use case which use 2 sentences in 
                                                                                        # single training example(for ex: Question-anwering) 
                                                                                        # we can have it as false
                                        padding                =  'max_length'  ,   # pad to longest sequence as defined by max_length
                                        return_attention_mask  =  True          ,   # Returns attention mask. Attention mask indicated 
                                                                                        # to the model which tokens should be attended to, 
        ...
        df_train = pd.concat([pd.DataFrame({'review'     : X_train['review_processed'].values,
                                            'review_old' : X_train['review'          ].values,
                                            }),
                            pd.DataFrame({'sentiment': y_train.values}),
                            ], axis = 1)


        df_valid = pd.concat([pd.DataFrame({'review'     :  X_valid['review_processed'].values,
                                            'review_old' :  X_valid['review'          ].values,
                                            }),
                            pd.DataFrame({'sentiment': y_valid.values}),
                            ], axis = 1)
        ...
        df_valid = pd.concat([pd.DataFrame({'review'    : X_valid['review'    ].values,
                                            'review_old': X_valid['review_old'].values,
                                            }),
                            pd.DataFrame({'sentiment': y_valid.values}),
                            ], axis = 1)


        df_test = pd.concat([pd.DataFrame({'review'    : X_test['review'    ].values,
                                        'review_old': X_test['review_old'].values,
                                        }),
                            pd.DataFrame({'sentiment': y_test.values}),
                            ], axis = 1)
                            '''
        ...
        batch_size = 50      # Bert recommendation
        ...
        # Lets build classifier for our reviews now. 
        # Below n_classes would be 2 in our case since 
        # we are classifying review as either positive or negative.
        ...
        def train_epoch(model, data_loader, optimizer, device, scheduler, n_examples):
            ...
            for idx, d in enumerate(data_loader):
                ...
                optimizer.step()                                            # After gradients are computed by loss.backward() this makes the optimizer 
                                                                                # iterate over all parameters it is supposed to update and use internally #stored grad to update their values
        ...
        for epoch in range(EPOCHS):
            ...
            # if val_acc > best_acc:
            #     torch.save(model.state_dict(), 'sentiment_11.pt')
            #     best_acc = val_acc
        torch.save(model.state_dict(), 'sentiment_'+ one_sentiment +'.pt')
